...
trait PasswordManagementTrait
{
    ...
    public function changePassword()
    {
        ...
        if (!empty($id)) {
            ...
            $user->id = $this->request->session()->read('Users.Key.Session.resetPasswordUserId');
    ...
    public function newPassword()
    {
        ...
        if (!empty($id)) {
            ...
            $user->id = $this->request->session()->read('Users.Key.Session.addPasswordUserId');
            //@todo add to the documentation: list of routes used
        }

        $this->set('validatePassword', false);
        ...
        if ($this->request->is('post')) {
            ...
            try {
                ...
                if ($user->errors()) {
                    ...
                    $user = $this->getUsersTable()->newPassword($user);
                    if ($user) {
                        $this->Flash->success(__d('Users', 'Password has been changed successfully'));
                        return $this->redirect($redirect);
                    } else {
     ...
     * @return void
     */
    public function addPassword($token = null)
    {
