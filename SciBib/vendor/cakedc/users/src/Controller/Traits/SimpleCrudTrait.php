...
trait SimpleCrudTrait
{
    ...
    public function add()
    {
        ...
        if ($table->save($entity)) {
            $this->Flash->success(__d('Users', 'The {0} has been saved', $this->request->data['username']));

            $resetUser = $this->getUsersTable()->addToken($this->request->data['username'], [
    ...
    public function edit($id = null)
    {
        ...
        if ($table->save($entity)) {
            ...
            $resetUser = $this->getUsersTable()->resetToken($this->request->data['email'], [
                'expiration' => Configure::read('Users.Token.expiration'),
                'checkActive' => false,
                'sendEmail' => true,
                'ensureActive' => true
