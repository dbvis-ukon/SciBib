            ...
            switch ($type) {
                ...
                case 'password':
                    ...
                    if (!empty($result)) {
                        ...
                        $this->request->session()->write('Users.Key.Session.resetPasswordUserId', $result->id);
                ...
                case 'add':
                    $result = $this->getUsersTable()->validate($token);
                    if (!empty($result)) {
                        $this->Flash->success(__d('Users', 'New password token was validated successfully'));
                        $this->request->session()->write('Users.Key.Session.addPasswordUserId', $result->id);
                        return $this->redirect(['action' => 'newPassword']);
                    } else {
                        $this->Flash->error(__d('Users', 'New password token could not be validated'));
                    }
                    break;
                default:
