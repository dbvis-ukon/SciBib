...
class PasswordBehavior extends Behavior
{
     ...
     * @throws UserAlreadyActiveException
     */
    public function addToken($reference, array $options = [])
    {
        if (empty($reference)) {
            throw new InvalidArgumentException(__d('Users', "Reference cannot be null"));
        }
        ...
        $user->updateToken($expiration);
        $saveResult = $this->_table->save($user);
        $template = !empty($options['emailTemplate']) ? $options['emailTemplate'] : 'CakeDC/Users.new_password';
        if (Hash::get($options, 'sendEmail')) {
            $this->Email->sendAddPasswordEmail($saveResult, null, $template);
        }
        return $saveResult;
    }
     ...
     * @return mixed
     */
    public function newPassword(EntityInterface $user)
    {
        $user = $this->_table->save($user);
        if (!empty($user)) {
            $user = $this->_removeValidationToken($user);
